select * FROM work.vasdata_work_service_change;

select count(*) FROM work.vasdata_work_service_change; -- 215.576.940
...
from (
  ...
  subscribed_product.Subscriber_no,
  subscribed_product.ban,
  ...
  subscribed_product.Subscriber_no,
  subscribed_product.ban,
  subscribed_product.effective_date  + interval 1 day as change_date
  from 
    analytics.abt_subscribed_product subscribed_product join
    base.import_fokus_base_service_agreement service_agreement on
      service_agreement.service_type in ('R','O','S') and
  ...
  from 
    base.import_fokus_base_pp_rc_rate pp_rc_rate join
    base.import_fokus_base_service_agreement service_agreement on
      pp_rc_rate.Soc = service_agreement.Soc and
      service_agreement.service_type in ('R','O','S') and
      service_agreement.soc = pp_rc_rate.soc and
  ...
  from 
    base.import_fokus_base_pp_rc_rate as pp_rc_rate join
    base.import_fokus_base_service_agreement as service_agreement on
      pp_rc_rate.Soc = service_agreement.Soc and
      service_agreement.service_type in ('R','O','S') and
      service_agreement.soc = pp_rc_rate.soc and
