select scp.`date`, scp.dealer_category, sum(scp.total_new_subs_sold), sum(scp.mtm_unconfirmed), sum(scp.mtm_given)
from analytics.abt_subscription_channel_performance scp
where  --scp.dealer_name = 'T-Shop - Fields 401065' and 
scp.`date` between '2019-03-01' and '2019-03-31'
group by scp.`date`, scp.dealer_category
order by scp.`date` desc
;


select sa.id sales_agent_id, sa.dealer_id, sa.email, d.dealer_group,d.dealer_name, di.dealer_name
-- select *  
from import_neo_base_sales_agent sa LEFT JOIN import_neo_base_ddb_dealer d 
on sa.dealer_id = d.id 
left join base.manual_files_base_man_dealer_info di
on cast(d.id as string) = di.dealer_code
order by di.dealer_name asc
;


select consent_id, count(*) 
...
select customer_id, subscriber_no, dealer_code from analytics.abt_subscriber_current
;


--select customer_id, dealer from analytics.abt_customer_current;


select *
from analytics.abt_subscription_channel_performance scp
where scp.mtm_unconfirmed > 0
;
...
left join
    ...
    where consent_id = 'f2225250-2b6d-4d8a-96d1-24155326282e'
...
select * from work.consent_tbt_consent_switchr_bridge scb
;


select * from base.import_consent_db_base_customer_consents_history 
;
...
SELECT cuen.customer_id, cuen.reference_type_id, count(*) c from  
base.import_consent_db_base_customer_entities cuen
group by cuen.customer_id, cuen.reference_type_id
order by c desc, cuen.customer_id
;


SELECT * FROM consent_tbt_clean_consent_data_btc
WHERE 
subscriber_id = 15497521
;


SELECT * 
from analytics.abt_consent_data_btc
where subscriber_id = 15497521
;

-- all consents enriched by customer info from Fokus
select * from work.consent_tbt_clean_consent_data_btc;


select * from customer_entity_group_entities gre where customer_entity_id in (101682684, 101553598);
...
and gre.customer_entity_group_id =  '001329c4-4c18-47b0-972b-ede9d40fd0ad' -- Tomasz
AND 
...
and
ge.customer_entity_id = cc.customer_entity_id
and ge.customer_entity_group_id = '001329c4-4c18-47b0-972b-ede9d40fd0ad' -- Tomasz group
order by ge.customer_entity_group_id, ge.customer_entity_id, cc.consent_id, cc.state_id
Limit 100
;
