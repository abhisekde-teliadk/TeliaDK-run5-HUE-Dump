select first_dealer_code from analytics.abt_subscriber_current;
...
select *
    from
    base.import_consent_db_base_customer_consents_history
where customer_entity_id = 101110660 and consent_id = 'f2225250-2b6d-4d8a-96d1-24155326282e'
;

select customer_entity_id, count(*)
    from
    base.import_consent_db_base_customer_consents_history
...
Select id as switchr_id, datecreated from analytics.abt_self_service_profiles
where switchr_id 

SELECT * FROM base.import_consent_db_base_customer_entities where reference_type_id = 'SWITCHR-ID' and 
...
According to Richard - SwitchR - has "MitTelia" username in Ninja/Fokus, Fokus id 10116
...
;
    ...
    SELECT usrban.userid, ban.bannumber
      ...
      WHERE 
        ban.id = usrban.banid
        AND usrban.userid = 1183
        ;


select * from analytics.abt_subscriber_current 
where customer_id in (471034116,551372212 )
;


SELECT * FROM base.import_self_service_base_user WHERE contactmsisdn IS NOT NULL  LIMIT 100;

SELECT * FROM base.import_self_service_base_role LIMIT 100;


SELECT * FROM base.import_self_service_base_authentication_log ORDER BY created DESC LIMIT 100;

---------------------------------------------------------
-- START: DML-187 #2 Number active subscriptions claimed
---------------------------------------------------------

SELECT count(*) claimed_sub_count
...
---------------------------------------------------------
-- END: DML-187 #2 Number active subscriptions claimed
---------------------------------------------------------
